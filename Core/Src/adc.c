...
void ADC_init( void ) { 
  ...
  ADC1->DIFSEL &= ~(ADC_DIFSEL_DIFSEL_6);    // PF3=ADC1_IN6, single-ended
  ...
  ADC1->SQR1  |= (6 << ADC_SQR1_SQ1_Pos);    // sequence = 1 conv., ch 5
  ADC1->SMPR1 |= (1 << ADC_SMPR1_SMP6_Pos);  // ch 5 sample time = 6.5 clocks
